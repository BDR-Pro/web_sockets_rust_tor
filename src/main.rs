...
pub async fn download_blockchain() -> Result<(), Box<dyn StdError>>  {
    ...
    async fn connect_via_socks_proxy(target_url: &str) -> Result<WebSocketStream<Socks5Stream<TcpStream>>, Box<dyn StdError>> {
        let proxy_addr = "socks5://127.0.0.1:9050";
        let target_url = Url::parse(target_url).expect("Invalid WebSocket URL");
        let stream = Socks5Stream::connect(proxy_addr, target_url.to_string()).await?; // Note: conversion to a string might be needed depending on your version of `tokio-socks`
    
        let (ws_stream, _) = tokio_tungstenite::client_async(Url::parse(&target_url)?, stream).await?;
...
async fn ping_onion_dns() -> Result<(), Box<dyn std::error::Error>> {
    //use proxy
    let result = send_a_message("".as_bytes(), ONION.to_string(), 3);
    match result {
        Ok(_) => {
...
pub async fn send_a_message(message:&[u8],receiver:String,type_message:i16) {
    // Assuming Tor is now installed and configured to listen on the default SOCKS5 port
    let proxy=tor_proxy();
...
pub async fn check_blockchain_for_the_node(text:String,receiver:String,write:WebSocketStream<TcpStream>) -> () {
    ...
    let my_blocks : i32 = count_files_in_folder("my_blocks").try_into().unwrap();
    ...
    if text.contains("/check") {
        ...
        if block_number < my_blocks {
            let response = send_a_message("The blockchain is ahead of you please sync. 201".to_string().as_bytes(),receiver,1).await;
            
        }
        else {
            let response = send_a_message("The blockchain is behind you please send blockchain data 404.".to_string().as_bytes(),receiver,1).await;
        ...
        if block_number < my_blocks {
            match tar_gz_your_blockchain(block_number+1,my_blocks) {
                Ok(bin) => {
                    let binary = bin.as_slice();
                    let response = send_a_message(binary,receiver,2).await;
